...
class JoystickWebSocketServer(QMainWindow):
    ...
    def setup_gui(self):
        ...
        settings_menu.addAction(
            QAction("config", self, triggered=self.open_settings_dialog)
        )
        settings_menu.addAction(
            QAction("key config", self, triggered=self.open_key_config_dialog)
        )
        settings_menu.addAction(
            QAction("update", self, triggered=lambda: self.check_updates(True))
        )
        settings_menu.addAction(
            QAction("reset counter", self, triggered=self.reset_counter)
        )
    ...
    def start_threads(self):
        ...
        # 皿処理スレッド
        self.density_thread = threading.Thread(target=self.thread_density, daemon=True)
        self.density_thread.start()

        # 計算用スレッド
        self.calc_thread = threading.Thread(target=self.thread_calc, daemon=True)
        self.calc_thread.start()

        # ボタン割当スクラッチの押しっぱなしを疑似的な連続回転として扱う
        self.axis_button_repeat_thread = threading.Thread(
            target=self.thread_axis_button_repeat, daemon=True
        )
        self.axis_button_repeat_thread.start()

        # 時計更新用スレッド
        self.uptime_thread = threading.Thread(target=self.thread_uptime, daemon=True)
    ...
    def open_settings_dialog(self):
        ...
        if self.settings.playmode == playmode.iidx_dp:
            ...
            if self.settings.connected_idx[1] is not None:
                ...
                self.set_label_text(
                    self.joystick_info[1], f"joypad disconnected", "red"
                )
    ...
    def change_joystick(self, controller_pos: int):
        ...
        if count < 1:
            ...
            if count == 1:
                if self.settings.playmode != playmode.iidx_dp:
                    if (self.settings.connected_idx[controller_pos] is None) or (
                        self.joystick[controller_pos] is None
                    ):
                    ...
                    if self.settings.connected_idx[1 - controller_pos] is not None:
                        ...
                        self.label_update_requested.emit(
                            self.joystick_info[1 - controller_pos],
                            f"joypad disconnected",
                            "red",
                        )
                ...
                if self.settings.playmode != playmode.iidx_dp:
                    ...
                    if self.joystick[controller_pos] is None:
                        ...
                        idx = (self.joystick[controller_pos].get_id() + 1) % count
                    if (
                        self.settings.connected_idx[1 - controller_pos] == idx
                    ):  # 他方のコントローラの割当を奪う場合
    ...
    def reconnect_joystick(self, controller_pos: int = 0, idx: int = 0):
        ...
        try:
            ...
            self.label_update_requested.emit(
                self.joystick_info[controller_pos],
                f"connected: {name} (ID: {idx})",
                "blue",
            )
                ...
                if count == 0:
                    ...
                    if self.settings.connected_idx[controller_pos] is not None:
                        self.reconnect_joystick(
                            controller_pos, self.settings.connected_idx[controller_pos]
                        )
                        logger.debug(
                            f"pos{controller_pos} connected (idx={self.settings.connected_idx[controller_pos]})"
                        )
                self.button_enabled_requested.emit(self.change_joystick_btn, True)
            except pygame.error as e:
                logger.error(e)
                self.label_update_requested.emit(
                    self.joystick_info[controller_pos], str(e), "red"
                )
    ...
    def is_valid_event(self, event):
        if self.has_key_config():
            if self.is_mapped_event(event) or event.type in (
    ...
    def has_key_config(self):
        mode_config = getattr(self.settings, "key_config", {}).get(
            self.settings.playmode.name, {}
        )
    ...
    def is_key_config_capturing(self):
        return (
            self.key_config_dialog is not None
            and self.key_config_dialog.capture_target_id is not None
        )
    ...
    def configured_target_ids(self):
        ...
        return {
            target_id
            for target_id, spec in mode_config.items()
            if spec and spec.get("event_type")
        }
    ...
    def default_target_is_configured(self, event):
        ...
        if (
            self.settings.playmode == playmode.iidx_dp
            and event.type == pygame.JOYAXISMOTION
            and event.axis == 0
        ):
        ...
        while True:
            try:
                for event in pygame.event.get():
                    if self.settings.debug_mode:
                        logger.debug(event)
                    if self.is_key_config_capturing() and self.is_registerable_event(
                        event
                    ):
                        spec = self.event_spec_from_pygame(event)
                        if spec:
                ...
                if pygame.joystick.get_count() == 0:
                    for i in range(2):
                        self.label_update_requested.emit(
                            self.joystick_info[i], f"joypad disconnected", "red"
                        )
            except Exception as e:
                logger.debug(traceback.format_exc())
            pygame.time.wait(20)

    def dispatch_event_data(
    ...
    def thread_axis_button_repeat(self):
        ...
        while True:
            with self.held_axis_button_lock:
                held_events = [
                    dict(event_data)
                    for event_data in self.held_axis_button_events.values()
                ]
            ...
            if target["kind"] == "button":
                if event.type == pygame.JOYBUTTONDOWN:
                    ...
                    self.dispatch_event_data(
                        event_data, count_notes=True, count_key=True
                    )
                    ...
                    if registered_sign is not None:
                        ...
                        active_direction = self.mapped_button_axis_direction.get(
                            state_key
                        )
                        pressed = abs(event.value) >= 0.5 and (
                            active_direction is None or direction == active_direction
                        )
                        if invert_axis:
                        ...
                        if invert_axis:
                            pressed = (
                                abs(event.value) >= 0.5
                                and direction != registered_direction
                            )
                        else:
                            pressed = (
                                abs(event.value) >= 0.5
                    ...
                    if previous_pressed == pressed or (
                        previous_pressed is None and not pressed
                    ):
                        continue
                    self.mapped_button_axis_state[state_key] = pressed
                    if not pressed:
                        self.mapped_button_axis_direction.pop(state_key, None)
                    event_data = target_to_event_data(
                        target, state="down" if pressed else "up"
                    )
                    self.dispatch_event_data(
                        event_data, count_notes=pressed, count_key=pressed
                    )
                ...
                count_other = target_id != self.pre_mapped_axis_direction.get(
                    (
                        registered_spec.get("controller_id"),
                        registered_spec.get("control_id"),
                    )
                )
                self.pre_mapped_axis_direction[
                    (
                ...
                self.dispatch_event_data(
                    event_data, count_notes=count_other, count_other=count_other
                )
            elif target["kind"] == "axis_dir" and event.type in (
                ...
                with self.held_axis_button_lock:
                    if is_down:
                        self.held_axis_button_events[hold_key] = target_to_event_data(
                            target
                        )
                    else:
                        self.held_axis_button_events.pop(hold_key, None)
                self.dispatch_event_data(
                    event_data, count_notes=is_down, count_other=is_down
                )
    ...
    def process_joystick_event(self, event):
        ...
        if event.type == pygame.JOYAXISMOTION:
            ...
            for i in range(2):
                if (
                    self.joystick[i] is not None
                    and self.joystick[i].get_instance_id() == event.instance_id
                ):
                    ...
                    logger.debug(
                        f"joypad {i} disconnected. connected_idx={self.settings.connected_idx}"
                    )
            if self.settings.playmode != playmode.iidx_dp:
                if pygame.joystick.get_count() > 0:
